# -*- coding: utf-8 -*-
"""
Most of the code in this file are Rob Romijnders
Explanation of the code at robromijnders.github.io/CNN_tsc

Added comments, additional printouts, saving and restoring network and some minor tweaks.
"""
...
"""Hyperparameters"""
# Change params here to tweak network
...
max_iterations = 10000 # Adjust if over- or underfitting, default is 10000
batch_size = 64
...
print('Will create a four layer CNN with %d %d %d %d filters/neurons' %
      (num_filt_1,num_filt_2,num_filt_3,num_fc_1))
...
bn_train = tf.placeholder(tf.bool,name = 'bn_train')          #Boolean value to guide batchnorm
...
with tf.name_scope("Evaluating_accuracy") as scope:
    prediction = tf.argmax(h_fc2,1,name='prediction')
    correct_prediction = tf.equal(tf.argmax(h_fc2,1), y_,name='correct_prediction')
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"),name='accacc')
...
with tf.Session() as sess:
  ...
  print('Saving the network as %s' % save_network_as)
  saver.save(sess, save_network_as)
...
print('Run tensorboard --logdir /your/path/time_series_classification/log_tb --port 6006 and point '
      'your browser to localhost:6006')
